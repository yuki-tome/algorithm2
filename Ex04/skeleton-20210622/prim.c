...
void Prim(int root, int n) {
    ...
    */
    ...
    for(i = 0; i < n; i++) {
        ...
        distance = INFTY;
        for(j = 0; j < n; j++) {
            if(visited[i]==0&&cost[i][v]<distance[i]/* Complete Here !! */) {
                distance = d[j];
                w = j;
            }
        }
        ...
        c = w;/* Complete Here !! */
            ...
            for(j = 0; j < n; j++) {
            if(label[j]==0&&cost[j]<d[j]/* Complete Here !! */) {
                d[j] = cost[j];/* Complete Here !! */       // The minimum distance to T
                    adj[j] = c;/* Complete Here !! */ // From which vertex c in T?
            }
        }
    }
}

...
void Dijkstra(int root, int n) {
    ...
    while(visited_order < n) { /* S !=V  -->  |V| < |S| */
        ...
        for(i = 0; i < n; i++) { // Choose a vertex w in V
            ...
            // such that...
            if( d[i] < min_cost /* [ Complete Here!! ] */) { // d[w] is a minimum;
                min_cost = d[i]/* [ Complete Here!! ] */;
                min_node = i/* [ Complete Here!! ] */;
            }
        }
        /*グラフが非連結の場合、次の行は不要*/
        if(min_cost == INFTY)
        ...
        for(i = 0; i < n; i++) { // for (v in V
            ...
            if( d[i] > d[min_node] + D[min_node][i]/* [ Complete Here!! ] */) {
                d[i] = d[min_node]+D[min_node][i]/* [ Complete Here!! ] */;
                adj[i] = min_node;
            }
        }
    }

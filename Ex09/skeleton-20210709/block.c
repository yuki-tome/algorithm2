...
void FindPlace(int **Board, int size, int h_x, int h_y) {
   ...
   int i,j,mid;
    ...
    if(h_x >= mid && h_y >= mid) A[mid-1][mid-1] = block_id;// ブロックが左上にないとき
    
    if(h_x < mid && h_y >= mid) B[mid-1][0] = block_id;// ブロックが右上にないとき
      
    if(h_x >= mid && h_y < mid) C[0][mid-1] = block_id; // ブロックが左下にないとき
      
    if(h_x < mid && h_y < mid) D[0][0] = block_id; // ブロックが右下にないとき
      
      block_id++;


      if(h_x >= mid && h_y >= mid) FindPlace(A,mid,mid-1,mid-1);

      else FindPlace(A,mid,h_x,h_y);

      if(h_x < mid && h_y >= mid) FindPlace(B,mid,0,mid-1);

      else FindPlace(B,mid,h_x-mid,h_y);

      if(h_x >= mid && h_y < mid) FindPlace(C,mid,mid-1,0);
      else FindPlace(C,mid,h_x,h_y-mid);

      if(h_x < mid && h_y < mid) FindPlace(D,mid,0,0);

      else FindPlace(D,mid,h_x-mid,h_y-mid);

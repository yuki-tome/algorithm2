...
int main() {
    ...
    for(i = 0; S[i] != '\0'; i++) {
        index[ S[i] ]++;/* [ Complete Here!! ] */; // Count the appearance of each character
    }
...
void construct_2(Node *A, int n) { // 無秩序に並んだ配列をヒープにする
    int i, j;
    for(i = n/2/* [ Complete Here!! ] */; i >= 1; i--) {
        downheap(A,i,n)/* [ Complete Here!! (Write function call) ] */;
    }
}
...
void downheap(Node *A, int k, int n) {
    ...
    while(k <= n / 2) {
        ...
        if(j < n) {
            if((A[j].count > A[j+1].count)/* [ Complete Here!! ] */ ||
               (A[j].count == A[j + 1].count && A[j].word > A[j + 1].word)) { //ヒープ条件の確認（Ex02とほぼ同じ）
                j++;
            }
        }

        if((v.count <= A[j].count)/* [ Complete Here!! ] */ || (v.count == A[j].count && v.word > A[j].word))
            break;
...
void upheap(Node *A, int k) {
    ...
    while(k > 1 && A[k/2].count >= v.count/* [ Complete Here!! ] */) { //ヒープ条件の確認（Ex02とほぼ同じ）
        A[k] = A[k / 2];
        k = k / 2;
    }
    ...
    while(1) {
        ...
        if(p->vertex.visited == 0) {
            if(p->vertex.articulation != -1)
                printf("%p [%3d]::  Left:%p [%3d]  Right:%p [%3d]\n", p, p->vertex.count, p->c_left,
                ...
                printf("%p [%3d]: <Terminal> %c  Code: %8s (%2dbits)\n", p, p->vertex.count, p->vertex.word,
        ...
        if((p->c_left != NULL) && (p->c_left->vertex.visited == 0)/* [ Complete Here!! ] */) { // DFSの要領で二分木を左側優先でたどる（もし、左側の子が探索可能で、かつ未到達であるならば。
                                        // Ex03とほぼ同じ）
            strcat(bin, "1");
            i++;
            totalbits = Huffman_DFS(p->c_left, bin);
        } else if(
            (p->c_right != NULL) && (p->c_right->vertex.visited == 0)/* [ Complete Here!! ] */) { // DFSの要領で二分木を右側にたどる（もし、右側の子が探索可能で、かつ未到達であるならば。
                                         // Ex03とほぼ同じ）
            strcat(bin, "0");
            i++;
